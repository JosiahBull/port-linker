...
fn main() {
    // Rerun if agent or its dependencies change
    watch_sources(&[
        "../agent/src",
        "../proto/src",
        "../scanner/src",
    ...
    // Configure the build
    let mut config = BuildConfig::from_env("agent")
        .target(
            CrossTarget::linux_x86_64()
                .with_custom_profile("agent-release".to_string()),
        )
        .target(
    ...
    for (target, result) in &results {
        match result {
            agent_build::BuildResult::Success { size, compressed, .. } => {
            ...
            agent_build::BuildResult::Failed { reason } => {
                eprintln!(

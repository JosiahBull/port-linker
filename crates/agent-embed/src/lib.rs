//! Embedded agent binaries for port-linker.
//!
//! This crate provides pre-compiled agent binaries for various target platforms.
...
mod binaries {
    ...
    pub static AARCH64_LINUX_MUSL: &[u8] = include_bytes!(concat!(
        env!("OUT_DIR"),
        "/agent-aarch64-unknown-linux-musl"
    ));

    /// macOS aarch64 (Apple Silicon)
    pub static AARCH64_DARWIN: &[u8] =
        include_bytes!(concat!(env!("OUT_DIR"), "/agent-aarch64-apple-darwin"));
}
...
/// Get the agent binary for the given operating system and architecture.
///
/// # Arguments
///
...
/// Get the agent binary for the given target info.
///

...
const AGENT_X86_64_LINUX_SHA256: &str = include_str!(concat!(
    env!("OUT_DIR"),
    "/agent-x86_64-unknown-linux-musl.sha256"
));

const AGENT_AARCH64_LINUX_SHA256: &str = include_str!(concat!(
    env!("OUT_DIR"),
    "/agent-aarch64-unknown-linux-musl.sha256"
));

const AGENT_AARCH64_DARWIN_SHA256: &str = include_str!(concat!(
    env!("OUT_DIR"),
    "/agent-aarch64-apple-darwin.sha256"
));

const AGENT_X86_64_WINDOWS_SHA256: &str = include_str!(concat!(
    env!("OUT_DIR"),
    "/agent-x86_64-pc-windows-msvc.sha256"
));

const AGENT_AARCH64_WINDOWS_SHA256: &str = include_str!(concat!(
    env!("OUT_DIR"),
    "/agent-aarch64-pc-windows-msvc.sha256"
));
...
/// The SHA256 of the uncompressed agent binary for the given OS and
...
pub fn get_binary_sha256_for_system(os: &str, arch: &str) -> Option<&'static str> {
    let hash = match (os.to_ascii_lowercase().as_str(), normalize_arch(arch)) {
        ("linux", "x86_64") => AGENT_X86_64_LINUX_SHA256,
        ("linux", "aarch64") => AGENT_AARCH64_LINUX_SHA256,
        ("macos" | "darwin", "aarch64") => AGENT_AARCH64_DARWIN_SHA256,
        ("windows", "x86_64") => AGENT_X86_64_WINDOWS_SHA256,
        ("windows", "aarch64") => AGENT_AARCH64_WINDOWS_SHA256,
        _ => return None,
    };

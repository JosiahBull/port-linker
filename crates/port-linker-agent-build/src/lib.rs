...
//!
//! ```rust,ignore
//! use port_linker_agent_build::{BuildConfig, CrossTarget, build_for_targets};
//!
//! fn main() {
//!     let config = BuildConfig::new("target-agent")
...
pub struct CrossTarget {
    ...
    /// Custom cargo profile to use (e.g., "target-agent-release")
...
mod tests {
    ...
    fn test_output_filename() {
        assert_eq!(
            output_filename("target-agent", "x86_64-unknown-linux-musl"),
    ...
    fn test_cross_target_custom_profile() {
        let target = CrossTarget::new("x86_64-unknown-linux-musl")
            .with_custom_profile("target-agent-release");
        assert_eq!(
            target.custom_profile,
            Some("target-agent-release".to_string())
        );
    ...
    fn test_build_config_binary_name() {
        let config = BuildConfig::new("target-agent").binary_name("agent");

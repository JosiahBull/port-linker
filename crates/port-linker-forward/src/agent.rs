//! Agent session for communicating with the remote target-agent.
//!
//! The `AgentSession` wraps an SSH channel connected to the deployed target-agent binary.
...
use port_linker_proto::{LogEvent, LogLevel, Message, ScanFlags, UdpPacket};
use port_linker_ssh::SshClient;
use port_scanner::{decode_remote_ports, BindAddress, RemotePort};
...
impl AgentSession {
    /// Deploy the target-agent binary and start a session.
    pub async fn deploy_and_start(client: &SshClient) -> Result<Self> {
        ...
        // Deploy the binary
        let path = format!("/tmp/target-agent-{}", std::process::id());
        info!(
            "Deploying target-agent to remote: {} (os={}, arch={}, size={})",
            path,
            os,
            arch,
            binary.len()

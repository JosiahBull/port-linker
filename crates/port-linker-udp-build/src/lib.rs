...
//!
//! ```rust,ignore
//! use port_linker_udp_build::{BuildConfig, CrossTarget, build_for_targets};
//!
//! fn main() {
//!     let config = BuildConfig::new("udp-proxy")
...
pub struct CrossTarget {
    ...
    /// Custom cargo profile to use (e.g., "udp-proxy-release")
...
impl CrossTarget {
    ...
    pub fn with_cross_fallback(mut self) -> Self {
...
impl BuildConfig {
    ...
    pub fn with_upx(mut self) -> Self {
        self.use_upx = true;
        self
    }

    /// Set whether this is a release build.
    pub fn release(mut self, is_release: bool) -> Self {
    ...
    pub fn from_env(package: impl Into<String>) -> Self {
...
impl BuildResult {
    ...
    pub fn path(&self) -> Option<&Path> {
        match self {
            BuildResult::Success { path, .. } => Some(path),
...
mod tests {
    ...
    fn test_output_filename() {
        assert_eq!(
            output_filename("udp-proxy", "x86_64-unknown-linux-musl"),

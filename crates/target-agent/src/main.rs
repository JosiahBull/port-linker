...
//! # Protocol
//!
//! Communication uses the port-linker-proto Message format over stdin/stdout.
//!
//! # Healthcheck
//!
...
use port_linker_proto::{Message, ScanFlags, UdpPacket};
use port_scanner::{encode_remote_ports, pick_scanner, Platform, PortScanner};
...
fn run() -> io::Result<()> {
    agent_debug!("Starting target-agent");
...
fn handle_scan_request(
    ...
    if flags.tcp {
        match scanner.scan(port_linker_proto::Protocol::Tcp) {
    ...
    if flags.udp {
        match scanner.scan(port_linker_proto::Protocol::Udp) {

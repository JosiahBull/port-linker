...
async fn test_relay_chain_authenticates_every_hop() {
    ...
    // Deployed in reverse, exactly as the CLI does it.
    let (mut relay2, relay2_port) = spawn_relay(&relay_bin, &agent_addr.to_string(), &secret);
    let (mut relay1, relay1_port) =
        spawn_relay(&relay_bin, &format!("127.0.0.1:{relay2_port}"), &secret);
    let entry: SocketAddr = format!("127.0.0.1:{relay1_port}").parse().unwrap();
    ...
    assert!(
        tokio::time::timeout(Duration::from_secs(1), agent.recv_from(&mut buf))
            .await
            .is_err(),
        "unauthenticated traffic crossed the relay"
    );
    ...
    let (len, relay2_src) = tokio::time::timeout(Duration::from_secs(5), agent.recv_from(&mut buf))
        .await
        .expect("registration did not propagate down the chain")
        .unwrap();
    ...
    let (len, _) = tokio::time::timeout(Duration::from_secs(5), agent.recv_from(&mut buf))
        .await
        .expect("client traffic did not reach the far end")
        .unwrap();
    assert_eq!(&buf[..len], b"through-the-chain");

    agent.send_to(b"reply", relay2_src).await.unwrap();
    let (len, _) = tokio::time::timeout(Duration::from_secs(5), client.recv_from(&mut buf))
        .await
